...
def build_messages(user_input, vm: VariableManager, prompt_config):
    ...
    if not result:
        print(f"WARNING: Empty result! message_blocks: {message_blocks}")
...
if __name__ == "__main__":
    # 新的YAML配置方式
    yaml_messages = build_messages(
        user_input="请继续创作故事...",
    )
    
    print(f"YAML方式生成了{len(yaml_messages)}条消息")
    for i, msg in enumerate(yaml_messages):
        print(f"\n=== YAML第{i+1}句 ({msg['role']}) ===")
        print(msg['content'])
        print("=" * 50)